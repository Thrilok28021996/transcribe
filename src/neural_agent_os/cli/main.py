...
from neural_agent_os import __version__
...
from neural_agent_os.infrastructure.logging import setup_logging
...
def identify_voice(container: ServiceContainer, audio_file: Path, name: str | None) -> None:
    ...
    embedding = speaker_id_engine._extract_voice_embedding(dummy_seg, audio_file)  # type: ignore[attr-defined]
...
@cli.command()
@click.argument("task_id", required=False, default="custom")
@click.option("--prompt", "-p", default=None, help="Prompt or parameter for task execution.")
@click.pass_obj
def agent_run(container: ServiceContainer, task_id: str, prompt: str | None) -> None:
    """Execute a personal agent task automation directly from terminal."""
    from neural_agent_os.application.services.agent_service import AgentService

    service = AgentService(
        storage_dir=str(container.config.storage.base_dir),
        llm_provider=container.get_llm_provider(),
    )

    console.print(f"[bold cyan]⚡ Executing Personal Agent Task: '{task_id}'...[/bold cyan]")
    result = service.execute_task(task_id=task_id, prompt=prompt)

    if result.status == "success":
        console.print(f"[bold green]✓ Task Completed ({result.execution_time_sec}s):[/bold green] {result.summary}")
        for log_line in result.logs:
            console.print(f"  [dim]{log_line}[/dim]")
        if result.artifacts:
            console.print(f"  [bold yellow]Artifact Generated:[/bold yellow] {', '.join(result.artifacts)}")
    else:
        console.print(f"[bold red]❌ Task Execution Failed:[/bold red] {result.summary}")


@cli.command()
@click.pass_obj
def assistant(container: ServiceContainer) -> None:
    """Interactive terminal AI Personal Assistant REPL."""
    from neural_agent_os.application.services.agent_service import AgentService
    ...
    console.print(Panel("[bold green]🤖 Neural Agent OS — Interactive Personal Assistant[/bold green]\nType any automation command, 'tasks' to view workflows, or 'exit'/'quit' to exit.", expand=False))
    ...
    while True:
        try:
            ...
            if user_input.lower() in ("tasks", "help"):
                tasks = service.get_available_tasks()
                console.print("[bold yellow]Available Automated Workflows:[/bold yellow]")
                for t in tasks:
                    console.print(f"  {t['icon']} [bold]{t['id']}[/bold]: {t['title']} — [dim]{t['description']}[/dim]")
                continue

            # Determine task ID from user input
            task_id = "custom"
            cmd_lower = user_input.lower()
            if "brief" in cmd_lower or "executive" in cmd_lower:
                task_id = "executive_briefing"
            elif "organize" in cmd_lower or "sort" in cmd_lower:
                task_id = "organize_files"
            elif "clean" in cmd_lower or "purge" in cmd_lower or "temp" in cmd_lower:
                task_id = "system_cleanup"
            elif "diag" in cmd_lower or "health" in cmd_lower:
                task_id = "system_diagnostics"
            elif "pdf" in cmd_lower or "convert" in cmd_lower or "doc" in cmd_lower:
                task_id = "convert_document"
            elif "remind" in cmd_lower or "speak" in cmd_lower or "say" in cmd_lower:
                task_id = "voice_reminder"
            elif "face" in cmd_lower or "biometric" in cmd_lower or "auth" in cmd_lower:
                task_id = "biometric_gate"
            elif "app" in cmd_lower or "keystroke" in cmd_lower or "window" in cmd_lower:
                task_id = "desktop_automation"

            console.print(f"[bold cyan][AGENT] Processing intent via task '{task_id}'...[/bold cyan]")
            res = service.execute_task(task_id=task_id, prompt=user_input)

            console.print(f"[bold green]Assistant >[/bold green] {res.summary}")
            if res.artifacts:
                console.print(f"  [dim]Artifact created: {', '.join(res.artifacts)}[/dim]")
...
@cli.command()
@click.argument("url")
@click.option("--duration", "-d", default=1800, help="Max duration in seconds.")
def bot_join(url: str, duration: int) -> None:
    """Send Vexa-style meeting bot to join a Google Meet, Teams, or Zoom call."""
    from neural_agent_os.infrastructure.vexa_bot import VexaMeetingBot

    bot = VexaMeetingBot()
    console.print(f"[bold cyan]🤖 Dispatching Vexa Meeting Bot to join URL: {url}[/bold cyan]")
    res = bot.join_meeting(meeting_url=url, duration_seconds=duration)

    console.print(f"[bold green]✓ Bot Status:[/bold green] {res['status']} ({res['platform']})")
    console.print(f"  [dim]Recording output: {res['audio_path']}[/dim]")


@cli.command()
def steno_monitor() -> None:
    """Start Steno-style background call detector for Zoom, Teams, and Google Meet."""
    from neural_agent_os.infrastructure.steno_passive_hook import StenoPassiveHook

    hook = StenoPassiveHook()
    console.print("[bold magenta]📡 Steno Passive Call Detector active on macOS...[/bold magenta]")
    status = hook.start_passive_monitoring()

    if status["detected_apps"]:
        console.print(f"[bold green]✓ Active meeting app detected:[/bold green] {', '.join(status['detected_apps'])}")
        console.print("[bold red]🔴 Loopback audio recording started automatically.[/bold red]")
    else:
        console.print("[dim]Monitoring running processes. No active calls right now.[/dim]")
