...
from neural_agent_os.domain.interfaces import (
    AlignmentEngine,
    DiarizationEngine,
    EmbeddingProvider,
    KnowledgeExtractor,
...
from neural_agent_os.infrastructure.config import AppConfig, load_config
from neural_agent_os.infrastructure.plugins.faster_whisper_plugin import (
    FasterWhisperSpeechRecognizer,
)
from neural_agent_os.infrastructure.plugins.llm_extractor import (
    LLMKnowledgeExtractor,
)
from neural_agent_os.infrastructure.plugins.lm_studio_plugin import (
    LMStudioLLMProvider,
)
from neural_agent_os.infrastructure.plugins.markdown_exporter import (
...
from neural_agent_os.infrastructure.plugins.speaker_id_plugin import (
    PersistentSpeakerIdentifier,
)
from neural_agent_os.infrastructure.speaker_store import SpeakerDatabase
...
class ServiceContainer:
    ...
    def _register_default_plugins(self) -> None:
        ...
        # Register real pyannote diarization if configured
        if self.config.diarization.hf_token:
            try:
                from neural_agent_os.infrastructure.plugins.pyannote_diarization_plugin import (
                    PyAnnoteDiarizationPlugin,
                )
                from neural_agent_os.infrastructure.adapters.pyannote_adapter import PyAnnoteDiarizationAdapter
                pyannote_plugin = PyAnnoteDiarizationPlugin(
                    hf_token=self.config.diarization.hf_token,
                    num_speakers=self.config.diarization.max_speakers,
                )
                pyannote_adapter = PyAnnoteDiarizationAdapter(plugin=pyannote_plugin)
                self.diarization_engines.register("pyannote", pyannote_adapter)
                logger.info("Registered pyannote.audio diarization engine.")
            except Exception as _pyannote_err:
                logger.debug(f"Could not register pyannote engine: {_pyannote_err}")
        ...
        from neural_agent_os.infrastructure.plugins.lm_studio_embedding import (
